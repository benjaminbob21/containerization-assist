 ...
 * via the AKS extension. They are not executed in unit tests, but several
...
describe('Skills integrity', () => {
  ...
  describe.each(skills)('skill: $folder', (skill) => {
    ...
    it('references only known tool names', () => {
      // Only inspect tokens that prose explicitly labels as a "tool" — either:
      //   - backticked form: "`build-image-context` tool" / "**`scan-image`** tool"
      //   - bold-with-suffix form: "**verify-deploy** (tool)"
      // This avoids false positives from unrelated kebab-case identifiers in
      // command snippets, table headings, etc.
      const re =
        /(?:\*{0,2}`([a-z][a-z0-9-]+)`\*{0,2}\s+tool|\*\*([a-z][a-z0-9-]+)\*\*\s+\(tool\))/gi;
