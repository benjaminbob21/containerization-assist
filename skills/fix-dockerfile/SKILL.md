...
argument-hint: <path to Dockerfile> [environment=production|development]
---
...
| `missing-oci-labels` | No `LABEL org.opencontainers.image.version` OR no `LABEL org.opencontainers.image.created-by` | info | Add OCI version/created-by labels for traceability. |
...
| `require-healthcheck` | Add `HEALTHCHECK CMD curl -fsS http://localhost:<port>/health \|\| exit 1` if image has a shell. For distroless, add a comment `# HEALTHCHECK: skipped — distroless image has no shell. Use Kubernetes liveness/readiness probe.` |
...
| `missing-oci-labels` | Add `LABEL org.opencontainers.image.created-by="containerization-assist"` and `LABEL org.opencontainers.image.version="<PACKAGE_VERSION>"` (use `containerization-assist-mcp/1.4.0` if unknown). |
...
| `path` doesn't exist or isn't readable | Echo path, ask user to verify. STOP. |
| File has no `FROM ` line | Reject: "This doesn't look like a Dockerfile." STOP. |
