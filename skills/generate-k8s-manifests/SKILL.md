...
| `appName` | yes | Kubernetes resource name. Must be a valid DNS-1123 label (lowercase, digits, `-`, ≤ 63 chars). If user gave something invalid, normalize it (lowercase, replace invalid chars with `-`) and tell them. |
| `image` | yes | Container image reference (`registry/repo:tag`). Required for `Deployment.spec.template.spec.containers[].image`. If not given, use `<appName>:dev` and note in output that the user must replace it before deploy. |
...
| `manifestType` | optional | `kubernetes` (default), `helm`, `kustomize`. **This skill only handles `kubernetes`.** For `helm` / `kustomize`, tell the user it's out of scope and STOP. |
| `outputDir` | optional | Where to write the YAML files. Default: `<modulePath>/k8s/`. |

If `modulePath` or `appName` is missing → ask the user. STOP.
...
Conditional: **ConfigMap** (any `config` / `database` env var with a
`defaultValue`), **Secret** (any `secret` env var — values **must be
placeholders**, never real secrets), **ServiceAccount** (managed DB
detected), **HorizontalPodAutoscaler** (production AND `trafficLevel ∈
{high, medium}`).
...
metadata:
  ...
  annotations:
    containerization-assist/version: containerization-assist-mcp/1.4.0
```
  ...
  template:
    metadata:
      labels:
        ...
        app.kubernetes.io/managed-by: containerization-assist
      annotations:
        # only if any secret env var exists:
        # azure.workload.identity/use: "true"
    spec:
      serviceAccountName: <appName>-sa          # omit if no managed DB
      securityContext:
        runAsNonRoot: true
        runAsUser: 10001
        fsGroup: 10001
      ...
      containers:
        - name: <appName>
          ...
          env:                                  # one entry per env var (see below)
            - name: PORT
              value: "<port>"
          envFrom:                              # only if ConfigMap or Secret exists
            - configMapRef: { name: <appName>-config }
            - secretRef:    { name: <appName>-secret }
          resources:
            requests: { cpu: …, memory: … }
            limits:   { cpu: …, memory: … }
          securityContext:
          ...
          livenessProbe:                        # only in production
            httpGet: { path: <probe path>, port: http }
            initialDelaySeconds: 30
            periodSeconds: 10
          readinessProbe:                       # only in production
            httpGet: { path: <probe path>, port: http }
...
apiVersion: v1
kind: ConfigMap
metadata: { …labels… name: <appName>-config }
data:
...
apiVersion: v1
kind: Secret
metadata: { …labels… name: <appName>-secret }
type: Opaque
stringData:
  <KEY>: "REPLACE_ME"
  ...
...
metadata:
  name: <appName>-sa
  namespace: <namespace>
  labels: { app.kubernetes.io/name: <appName>, app.kubernetes.io/managed-by: containerization-assist }
  annotations:
